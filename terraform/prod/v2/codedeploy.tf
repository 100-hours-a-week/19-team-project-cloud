...
resource "aws_codedeploy_app" "prod_v2_ai" {
  name             = "${local.name}-ai"
  compute_platform = "Server"
...
resource "aws_codedeploy_deployment_group" "prod_v2_ai" {
  app_name              = aws_codedeploy_app.prod_v2_ai.name
  deployment_group_name = "${local.name}-ai-dg"
  service_role_arn      = aws_iam_role.prod_v2_codedeploy.arn
